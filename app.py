...
generation_status = {}
app_config = {
    'model_path': 'janus_pro1b',
    'max_history': 20,  # Store status for the last 20 generations
    'example_prompts': [
        "A photo-realistic image of a futuristic city with flying cars",
        "A beautiful mountain landscape at sunset with a lake reflection",
        "A professional portrait photo of a person in a business suit",
        "A detailed image of a fantasy dragon in flight",
        "A cozy interior of a modern living room with large windows"
    ]
}
...
def load_model():
    ...
    try:
        ...
        if not gpu_available:
            print("WARNING: CUDA is not available. Using CPU instead (slower performance).")
            
        # Initialize the processor and tokenizer
        print(f"Loading processor from {model_path}...")
        vl_chat_processor = VLChatProcessor.from_pretrained(model_path)
        
        # Optimize model loading based on available hardware
        ...
        if gpu_available:
            ...
            # Use bfloat16 for better memory efficiency and potentially better performance
            vl_gpt = AutoModelForCausalLM.from_pretrained(
                model_path, 
                trust_remote_code=True,
                torch_dtype=torch.bfloat16,
                device_map="auto"  # Automatically use best GPU configuration
            )
            ...
            # CPU optimization - load in FP32 but with optimizations
            import multiprocessing
            num_threads = min(8, multiprocessing.cpu_count())  # Use up to 8 threads
            ...
            print("Model loaded to CPU. Note: Generation will be significantly slower.")
...
def generate_image_async(prompt_text, image_id, resolution="1K", cfg_weight=7.0, temperature=0.8, num_images=1):
    ...
    print(f"[INFO] Starting image generation for ID: {image_id}")
    ...
    # Keep only the most recent generations in status
    if len(generation_status) > app_config['max_history']:
        oldest_key = min(generation_status.keys(), key=lambda k: generation_status[k].get('started_at', 0))
        del generation_status[oldest_key]
    ...
    try:
        ...
        if is_gpu:
            ...
            # Determine max images based on resolution and GPU memory
            gpu_mem = torch.cuda.get_device_properties(0).total_memory / 1e9
            if resolution.upper() == "4K":
                max_images = max(1, min(4, int(gpu_mem / 4)))
            elif resolution.upper() == "2K":
                max_images = max(1, min(8, int(gpu_mem / 2)))
            else:
                max_images = max(1, min(16, int(gpu_mem)))
        else:
            # Limit to fewer images on CPU to prevent memory issues
            max_images = 2
        ...
        print(f"[INFO] Using device: {device} for generating {num_images} images at {resolution} resolution")
        ...
        for i in range(parallel_size):
            # Update status
            generation_status[image_id]["completed"] = i + 1
            ...
            if resolution.upper() != "1K":
                # Apply upscaling
                print(f"[INFO] Upscaling image {i+1}/{parallel_size} to {target_size}")
            ...
            print(f"[INFO] Saved image {i+1}/{parallel_size}")
        ...
        if not confirmed:
            ...
            generation_status[image_id] = {
                ...
                "settings": {
                    ...
                    "cfg_weight": cfg_weight,
                    "temperature": temperature,
                    "num_images": min(int(data.get('num_images', 1)), 
                                   16 if torch.cuda.is_available() else 2)
                },
...
def get_history():
    ...
    sorted_history = sorted(
        [status for status in generation_status.values() if 'started_at' in status],
...
@app.route('/api/example_prompts', methods=['GET'])
def get_example_prompts():
    """Return the list of example prompts"""
    return jsonify(app_config['example_prompts'])


@app.route('/api/process_example/<int:prompt_index>', methods=['POST'])
def process_example_prompt(prompt_index):
    """Process an example prompt by index"""
    if prompt_index < 0 or prompt_index >= len(app_config['example_prompts']):
    ...
    cfg_weight = float(data.get('cfg_weight', 7.0))
    temperature = float(data.get('temperature', 0.8))
    num_images = min(int(data.get('num_images', 1)), 4)  # Limit to 4 by default
    confirmed = data.get('confirmed', False)
    
    # Get the example prompt
    prompt = app_config['example_prompts'][prompt_index]
    
    try:
        # Generate a unique ID for this generation or use existing
        image_id = data.get('image_id', str(int(time.time() * 1000)))
        
        # Handle similar to api_generate
        if not confirmed:
            # Create entry but don't start processing
            generation_status[image_id] = {
                "status": "pending",
            ...
            return jsonify({
                "success": True,
                "message": f"Example prompt queued: \"{prompt}\"",
                "id": image_id,
                "status": "pending"
            })
        ...
        # If confirmed, start the generation process
            ...
            # Log the start of generation
            print(f"[INFO] Started manual example generation for prompt: \"{prompt}\"")
            print(f"[INFO] Example index: {prompt_index}")
            print(f"[INFO] Generation ID: {image_id}")
            print(f"[INFO] Settings: Resolution={resolution}, CFG={cfg_weight}, Temp={temperature}, Images={num_images}")
            
            return jsonify({
                "success": True,
                "message": f"Example generation confirmed and started",
                "id": image_id,
                "status": "processing"
            })
            
    except Exception as e:
        print(f"[ERROR] Error processing example prompt: {str(e)}")
        return jsonify({"error": str(e)}), 500
...
def confirm_generation(image_id):
    ...
    # Check if this was an example prompt
    if 'example_index' in settings:
        # Call the example prompt processing with confirmation
        example_index = settings['example_index']
        return process_example_prompt(example_index, request_data=confirmation_request)
    else:
